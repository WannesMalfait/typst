...
/// [argument sinks]($args). The right-hand side of the binding can be a block
/// or any other expression. It defines the function's return value and can
...
impl Func {
    ...
    pub fn title_name(&self) -> Option<&'static str> {
        match &self.repr {
            Repr::Native(native) => Some(native.title_name),
            Repr::Element(elem) => Some(elem.title_name()),
            Repr::Closure(_) => None,
            Repr::With(with) => with.0.title_name(),
        }
    }
