...
///
/// # Complex strokes
/// For full control, you can also pass a [dictionary]($dict) to any function
/// that expects a stroke. This dictionary has the following keys:
///
...
///
/// - `miter-limit`: Number at which protruding sharp angles are rendered with
///   a bevel instead. The higher the number, the sharper an angle can be
///   before it is bevelled. Only applicable if `join` is `{"miter"}`.
///   Defaults to `{4.0}`.
///
...
///     thickness.
///   - A [dictionary]($dict) with the keys `array` (same as the array above),
///     and `phase` (of type [length]($length)), which defines where in the
///     pattern to start drawing.
///
/// # Fields
