...
fn func_page(
    ...
    PageModel {
        ...
        title: func.title_name().unwrap().into(),
...
fn type_page(resolver: &dyn Resolver, parent: &str, ty: &Type) -> PageModel {
    ...
    PageModel {
        route: eco_format!("{parent}{}/", urlify(ty.name())),
        title: ty.title_name().into(),
        description: eco_format!("Documentation for the `{}` type.", ty.name()),
        part: None,
...
fn type_model(resolver: &dyn Resolver, ty: &Type) -> TypeModel {
    TypeModel {
        ...
        scope: scope_models(resolver, ty.name(), ty.scope()),
    }
}
