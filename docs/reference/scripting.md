...
- **Content block:** `{[*Hey* there!]}` \
  ...
  store it in variables and pass it to [functions]($func). Content
...
a [custom named function]($func/#defining-functions). Let bindings can be
...
[dictionaries]($dict). In this case, the left-hand side of the
...
- `{for pair in dict {..}}` \
  Iterates over the key-value pairs of the [dictionary]($dict).
...
can be either:
- a [dictionary]($dict) that has the specified key,
...
- [content]($content) consisting of an element that has the specified field. The
  ...
  [element function]($func/#element-functions) that were given when the element
  was constructed.
