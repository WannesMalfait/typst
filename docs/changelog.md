...
- Scripting
  ...
  - Improved accuracy of [`calc.pow`]($calc.pow) and [`calc.log`]($calc.log) for
    specific bases
  - Fixed [removal]($dict.remove) order for dictionary
...
- Scripting
  ...
  - The `at` methods on [strings]($str.at), [arrays]($array.at),
    [dictionaries]($dict.at), and [content]($content.at) now support specifying
    a default value
...
- Bug fixes
  ...
  - Fixed a bug with [argument sinks]($args)
...
- **Breaking changes:**
  ...
  - [Dictionaries]($dict) now iterate in insertion order instead of alphabetical
    order.
