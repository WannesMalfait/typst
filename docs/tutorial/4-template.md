...
The new `authors` parameter expects an [array]($array) of [dictionaries]($dict)
...
multiple arguments for the grid. We can do that by using the [`spread`
operator]($args). It takes an array and applies each of its items as a separate
